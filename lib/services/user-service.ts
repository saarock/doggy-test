...
import type { User, UserWithDistance, CreateUserInput, UpdateUserInput } from "@/lib/db/types"

export const userService = {
  // Create or update user on login
  async upsertUser(input: CreateUserInput): Promise<User> {
    const result = await sql`
      INSERT INTO users (id, google_id, email, name, avatar_url, age_confirmed, latitude, longitude, is_online)
      VALUES (${input.id}, ${input.google_id || null}, ${input.email}, ${input.name}, ${input.avatar_url || null}, ${input.age_confirmed}, ${input.latitude || null}, ${input.longitude || null}, true)
  ...
  async updateUser(id: string, input: UpdateUserInput): Promise<User | null> {
    ...
    const result = await sql`
      ...
      SET name = COALESCE(${input.name || null}, name),
          ...
          last_seen = CASE WHEN ${input.is_online ?? null} = true THEN NOW() ELSE last_seen END,
          updated_at = NOW()
      WHERE id = ${id}
      RETURNING *
    `
